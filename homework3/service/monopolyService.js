...
// function readAnyPlayers(res, next) {
//     db.many(`SELECT name
...
function readJoinQuery(req, res, next) {
    db.many(`SELECT name, score
                FROM Player, PlayerGame, Game
                WHERE Player.ID = PlayerGame.PlayerID
                AND Game.ID = PlayerGame.GameID
                AND PlayerGame.cash >= 4000
                AND Player.emailAddress LIKE '%gmail%'
    `)
    .then(data => {
        res.send(data);
    })
